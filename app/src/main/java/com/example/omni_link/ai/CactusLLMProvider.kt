...
""".trimIndent()
         ...
         *
         * @param modelPathOrSlug Either a Cactus model slug (e.g., "qwen3-0.6"), a local model
         * (e.g., "local-model"), or a path to a .gguf file
         */
        override suspend fun loadModel(modelPathOrSlug: String): Result<Unit> =
                withContext(Dispatchers.IO) {
                        try {
                                ...
                                // Determine if this is a slug (no path separators and doesn't end
                                // with known
                                // file extensions)
                                // Note: slugs CAN contain dots for version numbers (e.g.,
                                // "qwen3-0.6")
                                val looksLikeSlug =
                                ...
                                // Use shared instance if available (has download state), otherwise
                                // create new
                                if (cactusLM == null) {
                                ...
                                if (looksLikeSlug) {
                                        ...
                                        // Skip catalog verification for local models (they connect
                                        // to local server)
                                        if (!isLocal) {
                                                // Query SDK catalog to find the actual model slug
                                                try {
                                                        val models =
                                                                cactusLM?.getModels() ?: emptyList()
                                                        Log.d(
                                                                TAG,
                                                                "SDK catalog has ${models.size} models:"
                                                        )
                                                        models.forEach { m ->
                                                                Log.d(
                                                                        TAG,
                                                                        "  - ${m.slug} (downloaded=${m.isDownloaded})"
                                                                )
                                                        }
                                                        ...
                                                        var targetModel =
                                                                models.firstOrNull {
                                                                        it.slug == modelPathOrSlug
                                                                }

                                                        // If not found, try partial match (in case
                                                        // SDK uses different
                                                        // naming)
                                                        if (targetModel == null) {
                                                                // Try to find a model that contains
                                                                // our slug or vice versa
                                                                val baseSlug =
                                                                        modelPathOrSlug
                                                                                .replace("-", "")
                                                                                .replace(".", "")
                                                        ...
                                                        Log.w(
                                                                TAG,
                                                                "Could not query SDK catalog: ${e.message}"
                                                        )
                                                }
                                        } else {
                                                Log.d(
                                                        TAG,
                                                        "Local model detected - skipping catalog verification"
                                                )
                                        }

                                        // Initialize directly by slug (Cactus SDK handles local-
                                        // prefix internally)
                                        // 🚀 PERFORMANCE: Using optimized context size for Nothing
                                        // Phone 3
                                        Log.d(
                                                TAG,
                                                "Attempting to initialize model by slug: $actualSlug"
                                        )
                                        Log.d(
                                                TAG,
                                        ...
                                        cactusLM?.initializeModel(
                                                CactusInitParams(
                                                        model = actualSlug,
                                                        contextSize =
                                                                DEFAULT_CONTEXT_SIZE // Reduced for
                                                        // speed
                                                        )
                                        )
                                ...
                                if (modelFile == null || !modelFile.exists()) {
                                        Log.e(
                                                TAG,
                                                "Model not found at path and slug init failed: $modelPathOrSlug"
                                        )
                                ...
                                return@withContext try {
                                        ...
                                        Log.d(
                                                TAG,
                                                "Cactus model initialized from file name slug: $modelSlug"
                                        )
                                        Result.success(Unit)
                                } catch (e: Exception) {
                                        Log.e(
                                                TAG,
                                                "Cactus initialization error (legacy): ${e.message}",
                                                e
                                        )
        ...
        override suspend fun generateResponse(
                ...
                withContext(Dispatchers.IO) {
                        ...
                        try {
                                ...
                                // Use Cactus SDK for inference if available, otherwise fall back to
                                // smart
                                // response
                                val responseText =
                                        if (cactusAvailable && cactusLM != null) {
                                                try {
                                                        ...
                                                        val cactusMessages =
                                                                mutableListOf<CactusChatMessage>()
                                                        ...
                                                        // Add conversation history (limited to 2
                                                        // for maximum speed)
                                                        // 🚀 PERFORMANCE: Fewer history = faster
                                                        // context processing
                                                        conversationHistory.takeLast(2).forEach {
                                                                msg ->
                                                                cactusMessages.add(
                                                                        CactusChatMessage(
                                                                                content =
                                                        ...
                                                        // Add current user message with screen
                                                        // context
                                                        ...
                                                        // Generate completion
                                                        // 🚀 PERFORMANCE: Low temperature = faster
                                                        // sampling, more
                                                        // deterministic
                                                        // Use mutex to prevent concurrent native
                                                        // library access (causes SIGSEGV)
                                                        val result =
                                                        ...
                                                        if (!responseContent.isNullOrBlank()) {
                                                                ...
                                                                Log.w(
                                                                        TAG,
                                                                        "Cactus returned empty, using fallback"
                                                                )
                                                        ...
                                                        Log.e(
                                                                TAG,
                                                                "Cactus inference failed, using fallback",
                                                                e
                                                        )
                                                        generateSmartResponse(
                                                                userMessage,
                                                ...
                                                // Fallback to smart response when Cactus not
                                                // available
                                                generateSmartResponse(
                                                        userMessage,
                                                        screenState,
                                ...
                                Result.success(
                                        LLMResponse(
                                                ...
                                                actions = actionPlan,
                                                memoryUpdates =
                                                        extractMemoryUpdates(safeResponseText),
        ...
        override suspend fun generateSuggestions(
                ...
                withContext(Dispatchers.IO) {
                        try {
                                ...
                                Log.d(
                                        TAG,
                                        "Generating suggestions for screen: ${screenState.packageName}"
                                )
...
JSON array:"""
                ...
                if (!responseContent.isNullOrBlank()) {
                        ...
                        if (suggestions.isNotEmpty()) {
                                ...
                                return generateContextAwareFallback(
                                        screenState,
                                        screenContext,
                                        maxSuggestions
                                )
        ...
        private fun generateContextAwareFallback(
                ...
                val phonePattern =
                        Regex(
                                """(?:\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}|\d{10,11}"""
                        )
                ...
                return try {
                        ...
                        jsonArray.mapIndexedNotNull { index, element ->
                                try {
                                        ...
                                        val title =
                                                obj.get("title")?.asString?.takeIf {
                                                        it.isNotBlank()
                                                }
                                        val type =
                                                obj.get("type")?.asString?.takeIf {
                                                        it.isNotBlank()
                                                }
                                        ...
                                        // Support both "value" (from prompt) and "target" (legacy)
                                        // field names
                                        val value =
        ...
        private fun extractJsonArrayFromResponse(response: String): String? {
                ...
                if (arrayStart >= 0) {
                        ...
                        if (lastCompleteObjectEnd > arrayStart) {
                                val salvaged =
                                        trimmed.substring(arrayStart, lastCompleteObjectEnd + 1) +
                                                "]"
                                DebugLogManager.info(
                                        TAG,
        ...
        /**
         * Parse date and time strings into milliseconds since epoch Supports formats like
         * "2024-12-25", "tomorrow", "next Monday", "14:30", "2:30 PM"
         */
        ...
        /**
         * Extract potential event details from screen content using heuristics Looks for patterns
         * like titles, dates, times, locations
         */
        private fun extractEventDetailsFromScreen(screenContext: String): ExtractedEventDetails {
                ...
                for (line in lines) {
                        ...
                        if (lineLower.matches(Regex(""".*\d{1,2}[:/]\d{2}.*""")) && line.length < 10
                        )
                                continue
                        if (lineLower.contains("button") ||
                                ...
                                continue
                        if (line.length in 3..60 &&
                                        !lineLower.startsWith("at ") &&
                                        !lineLower.startsWith("on ")
                        ) {
                                title = line
                                break
                        }
        ...
        private fun generateSmartResponse(
                ...
                return when {
                        ...
                        lowerMessage.contains("what") &&
                                (lowerMessage.contains("screen") ||
                                        lowerMessage.contains("see")) -> {
                                val elements =
                                        screenState?.flattenElements()?.take(10)?.mapNotNull {
                                                it.getLabel()
                                        }
                                                ?: emptyList()
                                val description =
                        ...
                        lowerMessage.contains("go home") ||
                                lowerMessage.contains("home screen") -> {
                        ...
                        lowerMessage.contains("calculate") ||
                                lowerMessage.contains("calculator") -> {
                        ...
                        // Copy to clipboard
                        lowerMessage.contains("copy ") &&
                                !lowerMessage.contains("copy text to") -> {
                                val textToCopy =
                                        extractQuotedText(userMessage)
                        ...
                        lowerMessage.contains("sound settings") ||
                                lowerMessage.contains("volume settings") -> {
                        ...
                        lowerMessage.contains("location settings") ||
                                lowerMessage.contains("gps settings") -> {
        ...
        /**
         * Try to extract JSON object from response text. The model might include extra text before
         * or after the JSON.
         */
        ...
        /**
         * Clean up response text for display. Removes JSON formatting if present and extracts
         * readable content.
         */
                        ...
                        try {
                                val json =
                                        JsonParser.parseString(
                                                        extractJsonFromResponse(trimmed) ?: trimmed
                                                )
                                                .asJsonObject
        ...
        /**
         * Generate suggestions with streaming output so users can see AI thinking in real-time.
        ...
        override fun generateSuggestionsStreaming(
                ...
                try {
                        ...
                        val suggestionPrompt =
                                buildStreamingPrompt(screenContext, maxSuggestions, focusRegion)
                        ...
                        val suggestions =
                                if (responseContent.isNotBlank()) {
                                        val cleanedResponse = cleanAIResponse(responseContent)
                                        val parsed =
                                                parseSuggestionsFromAI(cleanedResponse, screenState)
...
JSON array:"""
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        // TEXT SELECTION OPTIONS GENERATION (Circle-to-Search)
        // ═══════════════════════════════════════════════════════════════════════════

        /**
         * Generate contextual options for selected text from OCR. Uses AI to understand the text
         * content and suggest relevant actions.
         */
        override fun generateTextOptions(
                selectedText: String,
                maxOptions: Int
        ): kotlinx.coroutines.flow.Flow<TextOptionStreamEvent> = callbackFlow {
                try {
                        if (selectedText.isBlank()) {
                                trySend(TextOptionStreamEvent.Error("No text selected"))
                                close()
                                return@callbackFlow
                        }
                        ...
                        Log.d(
                                TAG,
                                "Generating options for selected text: ${selectedText.take(50)}..."
                        )
                        DebugLogManager.info(
                                TAG,
                                "Text options generation started",
                                "Selected text (${selectedText.length} chars): ${selectedText.take(100)}..."
                        )

                        // First, generate smart fallback options based on text patterns
                        val smartOptions = generateSmartTextOptions(selectedText)

                        // If AI is not available, return smart options immediately
                        if (!cactusAvailable || cactusLM == null || !isModelLoaded) {
                                DebugLogManager.info(
                                        TAG,
                                        "AI not available, using smart text options"
                                )
                                trySend(
                                        TextOptionStreamEvent.Complete(
                                                smartOptions.take(maxOptions)
                                        )
                                )
                                close()
                                return@callbackFlow
                        ...
                        // Build the AI prompt for text options
                        val textOptionsPrompt = buildTextOptionsPrompt(selectedText, maxOptions)

                        DebugLogManager.prompt(
                                TAG,
                                "Streaming AI prompt for text options",
                                "Prompt length: ${textOptionsPrompt.length} chars"
                        )

                        val cactusMessages =
                                listOf(
                                        CactusChatMessage(
                                                content = textOptionsPrompt,
                                                role = "user"
                                        )
                                )
                        val fullResponse = StringBuilder()
                        ...
                        DebugLogManager.response(
                                TAG,
                                "Text options streaming complete",
                                "Response:\n$responseContent"
                        )

                        // Parse AI response into text options
                        val options =
                                if (responseContent.isNotBlank()) {
                                        val cleanedResponse = cleanAIResponse(responseContent)
                                        val parsed =
                                                parseTextOptionsFromAI(
                                                        cleanedResponse,
                                                        selectedText
                                                )
                                        if (parsed.isNotEmpty()) {
                                                ...
                                                // Fallback to smart options if parsing fails
                                                smartOptions
                                        }
                                } else {
                                        smartOptions
                        ...
                        trySend(TextOptionStreamEvent.Complete(options.take(maxOptions)))
                        close()
                } catch (e: Exception) {
                        Log.e(TAG, "Text options generation failed", e)
                        DebugLogManager.error(
                                TAG,
                                "Text options failed",
                                e.message ?: "Unknown error"
                        ...
                        // Return smart options as fallback on error
                        val fallbackOptions = generateSmartTextOptions(selectedText)
                        trySend(TextOptionStreamEvent.Complete(fallbackOptions.take(maxOptions)))
                        close()
                }
        ...
        /** Build the prompt for generating text options */
        private fun buildTextOptionsPrompt(selectedText: String, maxOptions: Int): String {
                return """You are a mobile assistant. The user selected this text from their screen:
...
JSON array:"""
        ...
        /** Parse AI response into TextOption list */
        private fun parseTextOptionsFromAI(
                response: String,
                selectedText: String
        ): List<TextOption> {
                return try {
                        val jsonString =
                                extractJsonArrayFromResponse(response) ?: return emptyList()
                        val jsonArray = JsonParser.parseString(jsonString).asJsonArray
                        ...
                        jsonArray.mapIndexedNotNull { index, element ->
                                try {
                                        ...
                                        val description = obj.get("description")?.asString ?: ""
                                        val type =
                                                obj.get("type")?.asString?.lowercase() ?: "search"
                                        val value = obj.get("value")?.asString ?: selectedText

                                        val action = createActionFromTextType(type, value)
                                        val icon = getTextOptionIcon(type)
                                        ...
                                        Log.e(TAG, "Failed to parse text option at index $index", e)
                                        null
                                }
                        }
                } catch (e: Exception) {
                        Log.e(TAG, "Failed to parse text options from AI", e)
                        emptyList()
                }
        }

        /** Create AIAction from text option type */
        private fun createActionFromTextType(type: String, value: String): AIAction {
                return when (type.lowercase()) {
                        "search" -> AIAction.WebSearch(value)
                        "copy" -> AIAction.CopyToClipboard(value)
                        "share" -> AIAction.ShareText(value)
                        "dial", "phone", "call" ->
                                AIAction.DialNumber(value.replace(Regex("[^\\d+]"), ""))
                        "sms", "message", "text" ->
                                AIAction.SendSMS(value.replace(Regex("[^\\d+]"), ""), "")
                        "email" -> AIAction.SendEmail(value, null, null)
                        "maps", "map", "location", "address" ->
                                AIAction.OpenMaps(query = value, navigate = false)
                        "navigate", "directions" ->
                                AIAction.OpenMaps(query = value, navigate = true)
                        "calendar", "event" -> AIAction.OpenCalendar(title = value)
                        "web", "url", "link" -> AIAction.OpenURL(value)
                        "translate" -> AIAction.WebSearch("translate $value")
                        "define", "definition" -> AIAction.WebSearch("define $value")
                        else -> AIAction.WebSearch(value) // Default to search
                }
        }

        /** Get icon for text option type */
        private fun getTextOptionIcon(type: String): TextOption.TextOptionIcon {
                return when (type.lowercase()) {
                        "search" -> TextOption.TextOptionIcon.SEARCH
                        "copy" -> TextOption.TextOptionIcon.COPY
                        "share" -> TextOption.TextOptionIcon.SHARE
                        "dial", "phone", "call" -> TextOption.TextOptionIcon.PHONE
                        "sms", "message", "text" -> TextOption.TextOptionIcon.SMS
                        "email" -> TextOption.TextOptionIcon.EMAIL
                        "maps", "map", "location", "address", "navigate", "directions" ->
                                TextOption.TextOptionIcon.MAP
                        "calendar", "event" -> TextOption.TextOptionIcon.CALENDAR
                        "web", "url", "link" -> TextOption.TextOptionIcon.WEB
                        "translate" -> TextOption.TextOptionIcon.TRANSLATE
                        "define", "definition" -> TextOption.TextOptionIcon.DEFINE
                        else -> TextOption.TextOptionIcon.INFO
                }
        }

        /** Generate smart text options based on patterns in the selected text */
        private fun generateSmartTextOptions(selectedText: String): List<TextOption> {
                ...
                if (addressIndicators.any { textLower.contains(it) } ||
                                text.matches(Regex(""".*\d+\s+\w+.*"""))
                ) {
                        options.add(
                                TextOption(
                                        title = "Open in Maps",
                ...
                // Detect date/time patterns (for calendar)
                val datePatterns =
                        listOf(
                ...
                options.add(
                        TextOption(
                                ...
                                action = AIAction.WebSearch(text)
                        )
                )
                ...
                // Add translate and define options for text that looks like words
                if (text.split(" ").size <= 5 &&
                                text.length <= 50 &&
                                !text.contains(Regex("""\d{3,}"""))
                ) {
                        options.add(
                                TextOption(
                                        title = "Translate",
